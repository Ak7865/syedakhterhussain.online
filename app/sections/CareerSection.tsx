    ...
    <section id="career" style={{ background: 'var(--bg-canvas)' }}>
      <div className="section-inner">
        ...
        <p style={{ fontSize: '15px', color: 'var(--text-secondary)', maxWidth: '540px', lineHeight: 1.75, marginBottom: '56px' }}>
          Real-world experience through internships, employment, and freelance work.
        </p>
        ...
        >
          {career.map((c, i) => (
            ...
            >
              ...
              <div style={{ fontSize: '10px', fontFamily: 'JetBrains Mono, monospace', color: 'var(--green)', letterSpacing: '1px', marginBottom: '7px' }}>{c.period}</div>
              <div className="tl-card-el" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', padding: '20px', transition: 'border-color 0.2s, transform 0.2s' }}>
                ...
                <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '3px' }}>{c.title}</h3>
                <h4 style={{ fontSize: '13px', color: 'var(--blue)', marginBottom: '10px' }}>{c.company}</h4>
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)', lineHeight: 1.7, marginBottom: '10px' }}>{c.desc}</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {c.tags.map((tag, ti) => (
                    <span key={tag} style={{
                      fontSize: '10px', padding: '3px 8px',
                      background: ti === c.tags.length - 1 && c.badge ? 'rgba(63,185,80,0.08)' : 'rgba(88,166,255,0.08)',
                      border: `1px solid ${ti === c.tags.length - 1 && c.badge ? 'rgba(63,185,80,0.2)' : 'rgba(88,166,255,0.2)'}`,
                      color: ti === c.tags.length - 1 && c.badge ? 'var(--green)' : 'var(--blue)',
