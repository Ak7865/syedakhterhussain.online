...
export default async function Image() {
  return new ImageResponse(
    (
      ...
      >
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(48,54,61,0.4) 1px, transparent 1px), linear-gradient(90deg, rgba(48,54,61,0.4) 1px, transparent 1px)', backgroundSize: '40px 40px', display: 'flex' }} />
        <div style={{ position: 'absolute', top: '-120px', left: '-120px', width: '500px', height: '500px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(63,185,80,0.18) 0%, transparent 70%)', display: 'flex' }} />
        <div style={{ position: 'absolute', bottom: '-100px', right: '-100px', width: '400px', height: '400px', borderRadius: '50%', background: 'radial-gradient(circle, rgba(88,166,255,0.15) 0%, transparent 70%)', display: 'flex' }} />
        ...
        <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', padding: '60px 72px', flex: 1 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '36px', marginBottom: '40px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'rgba(63,185,80,0.12)', border: '1px solid rgba(63,185,80,0.35)', padding: '4px 14px', borderRadius: '20px', width: 'fit-content' }}>
                ...
                <span style={{ color: '#3fb950', fontSize: '13px', letterSpacing: '2px' }}>Available for Freelance</span>
              </div>
              <div style={{ fontSize: '52px', fontWeight: 800, color: '#e6edf3', lineHeight: 1.05, display: 'flex' }}>Syed Akhter Hussain</div>
              <div style={{ fontSize: '24px', fontWeight: 600, display: 'flex' }}>
                <span style={{ color: '#3fb950' }}>Full Stack</span>
                <span style={{ color: '#8b949e', marginLeft: '10px' }}>Web Developer · Nagaon, Assam</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {['React.js', 'PHP', 'Node.js', 'MySQL', 'Firebase', 'JavaScript', 'Python', 'Java'].map((skill) => (
              <div key={skill} style={{ background: 'rgba(88,166,255,0.08)', border: '1px solid rgba(88,166,255,0.25)', padding: '6px 14px', borderRadius: '6px', color: '#58a6ff', fontSize: '14px', fontWeight: 600, display: 'flex' }}>{skill}</div>
            ))}
          </div>
        </div>
